...
total_watts=(selection_query["watts"].sum())
...
with first_column:
    ...
    st.subheader(f'{total_watts}GWH')
#with second_column:
   # st.markdown("### County")
...
st.markdown("---")
#Barchart
watts_by_category=(selection_query.groupby(by=["SubCounty"]).sum()[["watts"]])
...
#Piechart
watts_by_category_piechart=px.pie(watts_by_category, names=watts_by_category.index,values="watts",title="Watts % By Category",hole=.4,color=watts_by_category.index,color_discrete_sequence=px.colors.qualitative.Plotly_r)
...
st.line_chart(dataset, x="SubCounty", y="watts")
print(dataset)
